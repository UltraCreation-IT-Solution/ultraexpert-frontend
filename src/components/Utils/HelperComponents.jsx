      ...
      {data.map((temp, idx) => (
        <div className="">
          <div className="relative w-[95%] m-auto">
            <div className={slider===idx ? "absolute bottom-0 w-full flex items-center justify-center h-[8vw] md:h-[5.5vw] bg-black/45 tracking-wider  text-[2.5vw] md:text-[1.5vw] text-white font-bold":"hidden"}>{temp.title}</div>
            ...
            <img
              src={temp.src}
              key={idx}
              className={slider === idx ? "h-[50vw] md:h-[30vw] w-[100%] object-cover shrink-0 flex" : "hidden"}
            />
          </div>
          <p className={slider === idx ? "flex w-[95%] m-auto mt-[1.5vw] text-justify text-[2vw] md:text-[1.1vw] font-montserrat" : "hidden"}>{temp.info}</p>
        </div>
      ))} 
