...
export const AllFav = () => {
  ...
  const getAllFavData = async () => {
    ...
    try {
      const res = await axios.get("/customers/connect/?action=4", {
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${jsonData.access_token}`,
        },
      ...
      Object.keys(json).forEach((categoryName) => {
        const categoryFav = json[categoryName];
        categoryFav.forEach((item) => {
          const category = item.category.name;

          if (!categorizedFav[category]) {
            categorizedFav[category] = [];
          }

          categorizedFav[category].push({
      ...
      setAllFavData(
        Object.keys(categorizedFav).map((category) => ({
          category,
          favArray: categorizedFav[category],
        }))
      );
    } catch (error) {
      console.log(error);
